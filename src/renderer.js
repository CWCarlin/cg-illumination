...
class Renderer {
    constructor(canvas, engine, material_callback, ground_mesh_callback) {
        ...
        this.scenes = [
            ...
            {
                scene: new Scene(this.engine),
                background_color: new Color4(0.2, 0.01, 0.2, 1.0),
                materials: null,
                ground_subdivisions: [500, 500],
                ground_mesh: null,
                camera: null,
    ...
    createScene3(scene_idx) {
        ...
        // Create point light sources
        let light1 = new PointLight('light1', new Vector3(1.0, 1.0, 5.0), scene);
        light1.diffuse = new Color3(0, 1.0, 0);
        light1.specular = new Color3(1.0, 1.0, 1.0);
        current_scene.lights.push(light1);

                        // Create point light sources
        let light2 = new PointLight('light2', new Vector3(2.0, 1.0, 5.0), scene);
        light2.diffuse = new Color3(0, 0.6, 0.5);
        light2.specular = new Color3(1.0, 1.0, 1.0);
        current_scene.lights.push(light2);
        ...
        ground_mesh.scaling = new Vector3(20.0, 1.0, 20.0);
        ground_mesh.metadata = {
            mat_color: new Color3(0.10, 0.85, 0.55),
            mat_texture: white_texture,
            mat_specular: new Color3(0.0, 0.0, 0.0),
            mat_shininess: 1,
            texture_scale: new Vector2(1.0, 1.0),
        ...
        for (let i = 0; i <= 30; i ++) {
            // Create other models
            let box = CreateBox('box' + i, {size: 1}, scene);
            let angle = (i / 20) * Math.PI * 2; // Calculate angle for circular orientation
            let radius = 5.0;
            let x = radius * Math.cos(angle);
            let z = radius * Math.sin(angle);
            box.position = new Vector3(x, 3.0, z);
            box.metadata = {
                mat_color: new Color3(i/10, i/10, i/10),
                mat_texture: white_texture,
                mat_specular: new Color3(1.0, 1.0, 1.0),
                mat_shininess: 10,
                texture_scale: new Vector2(1.0, 1.0)
